...
slug: "build-benchmarks/explanation/data-preparation.html"
...
When you set `metadata_file`, you can attach tags to benchmark rows and filter few-shots with per-target `tags`, as described in [Getting the Right Questions From the Source Benchmark](/get-right-questions).
...
Authoritative identifiers and default subsets are listed in [Supported Hugging Face Benchmarks](/../reference/benchmarks).
...
Refer to [Output Files](/../reference/output-files) for column names.
...
- [Question Generation](/question-generation) shows how generate consumes `seed.parquet`.

- [Pipeline Overview](/pipeline-overview) situates prepare inside the wider `mcq` family.

- [Generation Configuration Reference](/../reference/generate-config) documents every YAML knob, including chunking and query defaults.
