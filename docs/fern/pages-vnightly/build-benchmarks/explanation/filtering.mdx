...
slug: "build-benchmarks/explanation/filtering.html"
...
Artifacts and filenames are listed in [Output Files](/../reference/output-files).
Field-level YAML is documented in [Generation Configuration Reference](/../reference/generate-config).
...
| <code>prompt_config</code> | <code>null</code> keeps packaged prompts; a path must supply <code>easiness_filter</code> and <code>hallucination_filter</code> blocks. Refer to [Prompt Tuning for Benchmarks](/../how-to/prompt-tuning). |
...
- [Quality Validation](/quality-validation) for upstream judgement, deduplication, distractors, and outlier stages that feed this step.

- [Question Generation](/question-generation) for how rows enter the generate pipeline before filtering.

- [Skip Stages When Iterating](/../how-to/skip-stages) if you need to rerun filtering after tweaking thresholds.
