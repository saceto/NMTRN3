...
slug: "build-benchmarks/how-to/domain-data.html"
...
Allowed `hf_dataset` values and default `subset` / `split` behavior are listed in [Supported Hugging Face Benchmarks](/../reference/benchmarks).
...
Field detail is in [Generation Configuration Reference](/../reference/generate-config).

## Next Steps

- Run prepare alone or the full pipeline after you align YAML: [Prepare Your Own Domain Data](/prepare-data).

- Inspect `seed.parquet` and downstream paths in [Output Files](/../reference/output-files).
