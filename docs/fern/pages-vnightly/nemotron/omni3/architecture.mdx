...
slug: "nemotron/omni3/architecture.html"
...
A deep-dive into the architectural decisions behind Nemotron 3 Nano Omni — what’s in the model, why each piece is there, and where the efficiency story comes from. For training instructions see the [recipe overview](/README); for deployment see [`inference.md`](/inference).
...
If you’re doing video-heavy inference and tuning for throughput, EVS configuration is the lever to optimize — see [`inference.md`](/inference) for runtime knobs.
...
[`docs/nemotron/omni3/sft.md`](/sft#config-variants) for the
...
Stage docs ([`sft.md`](/sft), [`rl.md`](/rl)) train and align this perception-sub-agent surface — instruction-following over multimodal inputs, not free-form chat.
