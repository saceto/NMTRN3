...
slug: "nemotron/omni3/inference.html"
...
How to deploy and serve Nemotron 3 Nano Omni after training (or against the public release checkpoint). For training instructions see the [recipe overview](/README); for architectural background see [`architecture.md`](/architecture).
...
- **EVS configuration**: for video-heavy workloads, the EVS layer’s frame-compression ratio is the most important throughput knob (see [`architecture.md` §Video pipeline](/architecture#video-pipeline-3d-convolutions-efficient-video-sampling-evs) for the why). Lower compression → higher quality, lower throughput; tune per workload.
...
- [`architecture.md`](/architecture) — the *why* behind the inference characteristics
