...
slug: "sdg/reference/troubleshooting.html"
...
Failure modes for local runs and cluster dispatch. For cluster-specific setup, see [Dispatch SDG to a Cluster](/../how-to/dispatch-to-cluster).
...
**Solution**: Check the spelling. For `person` and `datetime` sampler support, `step.py` must be extended — see the extension reference in [Config Schema](/config-schema).
...
**Solution**: Set `nemo_run_dir` to a path on the shared NFS mount and add the corresponding `mounts` entry to the env.toml profile. See [Dispatch SDG to a Cluster](/../how-to/dispatch-to-cluster).
...
- [Dispatch SDG to a Cluster](/../how-to/dispatch-to-cluster) — Full cluster setup walkthrough.

- [CLI Reference](/cli-reference) — Flags and hydra overrides.

- [Config Schema](/config-schema) — YAML field reference.
