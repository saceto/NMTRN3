...
description: "Sample prompts and a short guide for productive agent-assisted sessions with nemotron steps translation."
...
The mechanics of the `nemotron steps translation` are shown in {doc}`getting-started` and the how-to guides.
...
```text
I need help running nemotron steps translation for a multilingual supervised fine-tuning experiment.
...
Constraints:
- Use the checked-in config at src/nemotron/steps/translate/translation/config/default.yaml with -c default and CLI dotlist overrides only.
...
Please:
- Give me the exact uv run nemotron steps translation command with input_path, output_dir, source_language=en, target_language=[TARGET], and server.model filled in.
