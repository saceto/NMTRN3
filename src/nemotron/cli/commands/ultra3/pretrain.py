...
"""Pretrain command implementation for Ultra3.

This module defines the `pretrain` command with visible NeMo-Run execution
logic. The runtime script wraps Megatron-Bridge's Ultra pretrain recipe; this
file only handles config compilation and local/cloud/Slurm submission.
"""
...
META = RecipeMeta(
    ...
    input_artifacts={"data": "Tokenized pretrain data (bin/idx) from `ultra3 data prep pretrain`"},
...
def _execute_pretrain(cfg: RecipeConfig, *, experiment=None):
    ...
    # Allow config to override the train script if needed for tests/forks.
    script_path = SCRIPT_PATH
