...
# [tool.runspec]
# schema = "1"
# name = "data/sdg/long-document/judge"
...
DEFAULT_CONFIG_PATH = Path(__file__).resolve().parent / "config" / "09-judge.yaml"


class JudgeConfig(BaseModel):
    """Pydantic config for the long-document frontier judge stage."""

    model_config = ConfigDict(extra="forbid")

    seed_path: Path = Field(
        ...,
        description="Path to the QA-output parquet file produced by an upstream stage "
                    "(e.g. 05/06/07/08).",
    )
    model_id: str = Field(
        ...,
        description="Frontier model id (e.g. gemini-2.5-flash-preview-04-17).",
    )
    endpoint: str = Field(
        ...,
    ...
    model_alias: str = Field(
        default="frontier-judge-vlm",
        description="Alias under which the frontier model is registered.",
    )
    num_records: int = Field(default=5, gt=0, description="Number of records to judge.")
...
def main(cfg: JudgeConfig | None = None) -> None:
    ...
    if cfg is None:
        cfg = load_recipe_config(DEFAULT_CONFIG_PATH, JudgeConfig)
